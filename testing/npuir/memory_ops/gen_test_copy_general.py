...
def gen_shape_rank_kernel_str(
    ...
    base_name = func_name.replace("kernel_", "").replace("_GM_UB_GM", "").replace("_UB_UB", "")
    ...
    if direction == "GM_UB_GM":
        lines = [
            ...
            "    @T.prim_func",
            f"    def {func_name}(",
            f"        A: T.Tensor(src_shape, dtype), B: T.Tensor(src_shape, dtype)",
            f"    ):",
            description_str,
        ...
        lines = [
            ...
            "    @T.prim_func",
            f"    def {func_name}(",
            f"        A: T.Tensor(src_shape, dtype), B: T.Tensor(dst_shape, dtype)",
            f"    ):",
            description_str,
            "        with T.Kernel(1, is_npu=True):",
...
""")
        ...
        for direction in DIRECTIONS:
            for dyn_case in DYNAMIC_SCENARIOS:
                ...
                f.write(gen_dynamic_kernel_str(dyn_case, direction, func_name, src_shape, ub_shape) + "\n\n")
